...
function checkWin(){if(state.foundations.flat().length===52){message('축하합니다! 모든 카드를 정리했습니다. 뉴스도 완전 당첨!');$('#prize').scrollIntoView({behavior:'smooth'});drawNews()}}
...
function openReward(){state.rewardOpen=true;showNews(state.news[state.newsIndex%state.news.length]||FALLBACK[0]);$('#prizeCard').classList.add('is-open');$('#stuckBtn').hidden=true;$('#drawAgainBtn').hidden=false;message('당첨! 오늘의 뉴스 한 장이 열렸습니다.');setTimeout(()=>$('#prize').scrollIntoView({behavior:'smooth',block:'start'}),500)}
function drawCategoryNews(fi){const category=state.categories[fi],pool=state.newsByCategory.get(category)||state.news;const item=pool[Math.floor(Math.random()*pool.length)]||FALLBACK[0];showNews(item);$('#prizeCard').classList.add('is-open');$('#prize').scrollIntoView({behavior:'smooth',block:'start'});message(`${SUITS[fi].symbol} ${category} 완성! 해당 카테고리 뉴스가 열렸습니다.`);tone(820)}
